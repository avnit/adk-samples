...
| [Auth Provider Demo](auth-provider-demo) | Demonstrates obtaining API access tokens via a shared auth provider abstraction, covering both 2-legged (client_credentials) and 3-legged (authorization_code + PKCE) OAuth2 flows. |   Auth provider, OAuth2, Function tool (Python), Tests | Tooling | Easy | Single Agent | Horizontal                    |
